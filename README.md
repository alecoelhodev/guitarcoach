...
- **Liveness vs. readiness split.** `health/live` reports process-up-ness only (no checks). `health/ready` additionally checks heap/RSS memory, disk usage, and Cloud SQL connectivity (a Prisma ping) via Terminus, matching the Docker/Compose `HEALTHCHECK` which polls `/health/ready` — and giving the deploy workflow's post-deploy smoke test (see [Continuous deployment](#continuous-deployment)) a real signal that the deployed revision can actually reach its database, not just that the process booted.
